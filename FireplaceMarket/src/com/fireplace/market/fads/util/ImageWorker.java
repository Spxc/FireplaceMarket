...
package com.fireplace.market.fads.util;
...
public abstract class ImageWorker {
	...
	public void loadImage(Object data, ImageView imageView, Boolean local) {
		...
		if (bitmap != null) {
			...
			final BitmapLocalWorkerTask task = new BitmapLocalWorkerTask(
					imageView);
	 ...
	 *            {@link ImageWorker#loadImage(Object, ImageView)}
	 * @param local
	 *            If accessing the local file system.
	 * @return The processed bitmap
	 */
	protected abstract Bitmap processBitmap(Object data, Boolean local);
	 ...
	 *         this imageView. null if there is no such task.
	 */
	private static BitmapLocalWorkerTask getBitmapLocalWorkerTask(
			ImageView imageView) {
		if (imageView != null) {
			final Drawable drawable = imageView.getDrawable();
	 ...
	 */
	private class BitmapWorkerTask extends JbAsyncTask<Object, Void, Bitmap> {
		private Object data;
	 ...
	 * The actual AsyncTask that will asynchronously process the image.
	 */
	private class BitmapLocalWorkerTask extends
			JbAsyncTask<Object, Void, Bitmap> {
		private Object data;
		private final WeakReference<ImageView> imageViewReference;
		...
		protected Bitmap doInBackground(Object... params) {
			if (FireplaceApplication.DEBUG) {
				Log.d(TAG, "doInBackground - starting local work");
			}

			data = params[0];
			String nullCheck = String.valueOf(data);
			final String dataString = (!nullCheck.endsWith("null.jpg")) ? nullCheck
					: null;
			Bitmap bitmap = null;
			...
			if (bitmap == null && !isCancelled()
					&& getAttachedImageView() != null && !mExitTasksEarly
					&& dataString != null) {
				bitmap = processBitmap(dataString, true);
			}
			...
			// add the processed
			// bitmap to our cache as it might be used again in the future
			if (bitmap != null && mImageCache != null && dataString != null) {
				mImageCache.addBitmapToCache(dataString, bitmap);
			}
		...
		private ImageView getAttachedImageView() {
			final ImageView imageView = imageViewReference.get();
			final BitmapLocalWorkerTask bitmapWorkerTask = getBitmapLocalWorkerTask(imageView);
	 ...
	 * finish order.
	 */
	private static class AsyncLocalDrawable extends BitmapDrawable {
		private final WeakReference<BitmapLocalWorkerTask> bitmapWorkerTaskReference;

		public AsyncLocalDrawable(Resources res, Bitmap bitmap,
				BitmapLocalWorkerTask bitmapWorkerTask) {
			super(res, bitmap);
			bitmapWorkerTaskReference = new WeakReference<BitmapLocalWorkerTask>(
					bitmapWorkerTask);
		}

		public BitmapLocalWorkerTask getBitmapWorkerTask() {
			return bitmapWorkerTaskReference.get();
		}
	}
	...
	protected class CacheAsyncTask extends JbAsyncTask<Object, Void, Void> {
